...
const KalenderRange = ({onDataReady, adaSuratDokter, iniFormSakit}) => {
  const [selectedStartDate, setSelectedStartDate] = useState(null);
  const [selectedEndDate, setSelectedEndDate] = useState(null);
  const [dataTglLibur, setDataTglLibur] = useState([]);
  const [dataTglCutiBersama, setDataTglCutiBersama] = useState([]);
  const [isFormSakit, setIsFormSakit] = useState(false);
  const [isSuratDokter, setIsSuratDokter] = useState(true);
  ...
  useEffect(() => {
    if (iniFormSakit) {
      setIsFormSakit(iniFormSakit);
    }
    if (isFormSakit) {
      setIsSuratDokter(adaSuratDokter);
    }
  }, [adaSuratDokter, iniFormSakit, isFormSakit]);
  console.log('ada surat dokter : ', adaSuratDokter);
  ...
  const onDateChange = (date, type) => {
    if (type === 'END_DATE') {
      ...
      isSuratDokter ? setSelectedEndDate(null) : setSelectedEndDate(date);
    }
  };
  ...
  return (
    <View style={styles.container}>
      <CalendarPicker
        ...
        allowRangeSelection={isSuratDokter}
        minDate={minDate}
        maxDate={maxDate}
